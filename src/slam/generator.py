...
class Config(models.Model):
    ...
    def createconf(self, hosts):
        ...
        duplicates = []
        if self.checkfile:
            duplicates = self.is_unique(hosts)
...
class BindConfig(Config):
    ...
    def _unique_host(self, line, host, addrs):
        ...
        records = line.split()
        for addr in addrs:
            if addr.addr and addr.addr in records:
                return False

        name = records[0]
        return (not name == host.name and not ("." in name
                and host.name == name[:name.find(".")]))
...
class RevBindConfig(Config):
    ...
    def _unique_host(self, line, host, addrs):
        ...
        name = records[-1]
        return (name != host.name and not ("." in name
                and host.name == name[:name.find(".")]))
...
class QuattorConfig(Config):
    ...
    def _unique_host(self, line, host, addrs):
        ...
        for addr in addrs:
            if addr.addr and addr.addr in line:
                return False
        return (name[1] != host.name and not ("." in name[1]
                and host.name == name[1][:name[1].find(".")]))
...
class DhcpdConfig(Config):
    ...
    def _unique_host(self, line, host, addrs):
        ...
        records = line.split()
        for addr in addrs:
            if addr.macaddr and (addr.macaddr in records
        ...
        name = line[line.find("host"):].split()
        if not len(name) >= 2:
            return True
...
class LalDnsConfig(Config):
    ...
    def _unique_host(self, line, host, addrs):
        ...
        return host.name not in records
